object FrmTanqueCombustivel: TFrmTanqueCombustivel
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  Caption = 'Cadastro de Tanque de Combust'#237'vel'
  ClientHeight = 392
  ClientWidth = 557
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  Position = poMainFormCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 15
  object pnlEdicao: TPanel
    Tag = 1
    Left = 0
    Top = 253
    Width = 557
    Height = 139
    Align = alBottom
    BevelInner = bvRaised
    BevelOuter = bvLowered
    BevelWidth = 3
    Enabled = False
    TabOrder = 0
    ExplicitTop = 252
    ExplicitWidth = 553
    object Label1: TLabel
      Left = 16
      Top = 16
      Width = 36
      Height = 15
      Caption = 'Nome:'
    end
    object Label2: TLabel
      Left = 16
      Top = 53
      Width = 70
      Height = 30
      Caption = 'Tipo de Combust'#237'vel:'
      WordWrap = True
    end
    object Label3: TLabel
      Left = 16
      Top = 90
      Width = 65
      Height = 30
      Caption = 'Capacidade em Litros:'
      WordWrap = True
    end
    object edtCapacidadeEmLitros: TEdit
      Left = 94
      Top = 87
      Width = 83
      Height = 23
      Alignment = taRightJustify
      TabOrder = 2
      Text = '0,00'
      OnKeyPress = edtCapacidadeEmLitrosKeyPress
    end
    object cbbTipoCombustivel: TComboBox
      Left = 94
      Top = 50
      Width = 83
      Height = 23
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 1
      Items.Strings = (
        ''
        'Diesel'
        'Gasolina')
    end
    object edtNome: TEdit
      Left = 94
      Top = 13
      Width = 339
      Height = 23
      MaxLength = 30
      TabOrder = 0
    end
  end
  object pnlBotoes: TPanel
    Left = 0
    Top = 0
    Width = 557
    Height = 57
    Align = alTop
    BevelInner = bvLowered
    BevelWidth = 2
    BorderStyle = bsSingle
    TabOrder = 1
    ExplicitWidth = 553
    DesignSize = (
      553
      53)
    object btnIncluir: TSpeedButton
      Tag = 1
      Left = 3
      Top = 12
      Width = 85
      Height = 33
      Anchors = [akTop, akRight]
      Caption = '&Incluir'
      Glyph.Data = {
        
        9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = btnIncluirClick
      ExplicitLeft = 291
    end
    object btnGravar: TSpeedButton
      Tag = 2
      Left = 258
      Top = 12
      Width = 85
      Height = 33
      Anchors = [akTop, akRight]
      Caption = '&Gravar'
      Enabled = False
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = btnGravarClick
      ExplicitLeft = 546
    end
    object btnAlterar: TSpeedButton
      Tag = 1
      Left = 88
      Top = 12
      Width = 85
      Height = 33
      Anchors = [akTop, akRight]
      Caption = '&Alterar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = btnAlterarClick
      ExplicitLeft = 376
    end
    object btnFechar: TSpeedButton
      Tag = 1
      Left = 428
      Top = 12
      Width = 85
      Height = 33
      Anchors = [akTop, akRight]
      Caption = '&Fechar'
      Glyph.Data = {
        
        F8F8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = btnFecharClick
      ExplicitLeft = 716
    end
    object SpeedButton1: TSpeedButton
      Tag = 2
      Left = 343
      Top = 12
      Width = 85
      Height = 33
      Anchors = [akTop, akRight]
      Caption = '&Cancelar'
      Enabled = False
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      OnClick = SpeedButton1Click
      ExplicitLeft = 631
    end
    object btnExcluir: TSpeedButton
      Tag = 1
      Left = 173
      Top = 12
      Width = 85
      Height = 33
      Anchors = [akTop, akRight]
      Caption = '&Excluir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -12
      Font.Name = 'Segoe UI'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentFont = False
      OnClick = btnExcluirClick
      ExplicitLeft = 461
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 57
    Width = 557
    Height = 176
    Align = alTop
    Anchors = [akLeft, akTop, akRight, akBottom]
    DataSource = DataSource
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ParentFont = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -12
    TitleFont.Name = 'Segoe UI'
    TitleFont.Style = [fsBold]
  end
  object DataSource: TDataSource
    OnDataChange = DataSourceDataChange
    Left = 224
    Top = 120
  end
end
